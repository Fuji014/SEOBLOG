...
export const userSignOut = () => async (dispatch) => {
  try {
    dispatch({
      type: userConstants.USER_SIGNOUT_REQUEST,
    });
    ...
    dispatch({
      type: userConstants.USER_SIGNOUT_SUCCESS,
      payload: res.data,
    });
  } catch (error) {
    dispatch({
      type: userConstants.USER_SIGNOUT_FAILURE,
      payload: {
        error:
          error.response && error.response.data.message
            ? error.response.data.message
...
export const userSignOutReset = () => (dispatch) => {
  dispatch({
    type: userConstants.USER_SIGNOUT_RESET,
  });
};
